...
let handler = async (m, { conn,isOwner,q  }) => {





  const groupFolderPath = path.join('./plugins',  q.charAt(0).toUpperCase() + q.slice(1).toLowerCase())

 

  function displayGroupCommands(folderPath) {
    let result = `📂 *Fitur Group*\n\n`
    try {
      const files = fs.readdirSync(folderPath)
      files.forEach(file => {
        if (path.extname(file) === '.js') {
          const fileName = path.parse(file).name
          result += `• ${transformText(fileName)}\n`
        }
      })
    } catch (e) {
      console.error(e)
      result = 'Gagal membaca folder Group!'
    }
    return result
  }

  const output = displayGroupCommands(groupFolderPath)

 if(q) return m.reply(output)
