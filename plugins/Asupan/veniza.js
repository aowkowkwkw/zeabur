...
let handler = async (m, { q, conn,setReply }) => {
  setReply(mess.wait)
  const total = shuffledAsupan.length

  if (!q) {
    // Kirim acakan random
    const random = pickRandom(shuffledAsupan)
    await conn.sendFile(m.chat, random, 'asupan.mp4', `_Nih Kak_`, m)
  } else {
    const num = parseInt(q)
    if (!isNumber(num) || num < 1 || num > total)
      throw `Pilih angka 1 sampai ${total}, contoh: .veniza 5`
    
    await conn.sendFile(m.chat, shuffledAsupan[num - 1], 'asupan.mp4', `Veniza ${num}`, m)
  }
}
